...
# Определяем путь к папке, где находится .exe или .py
if getattr(sys, 'frozen', False):  
    BASE_DIR = os.path.dirname(sys.executable)  # Если запущен .exe
else:
    BASE_DIR = os.path.dirname(os.path.abspath(__file__))  # Если запущен .py
...
user_video_notes = {}  # {user_id: file_id} - Видеокружо
...
async def send_broadcast(message: types.Message):
    ...
    users = await get_users()  # ✅ Теперь берем пользователей из базы
    success_count = 0
    fail_count = 0

    for user in users:
        try:
            await bot.send_message(user["id"], text)  # ✅ Берем user["id"], а не user['id']
            success_count += 1
...
async def show_users(message: types.Message):
    ...
    users = await get_users()  # ✅ Теперь загружаем список пользователей из базы
    if not users:
